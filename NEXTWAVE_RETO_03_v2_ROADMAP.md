...
- `dev` demostrable siempre; merge obligatorio en cada gate.
...
| 3 Loop humano + LLM (3.1–3.10) | 🟡 | Código completo: pausa `DECISION_REQUIRED`, policy + rechazos, `LLMComposer` (Responses API, structured output, timeout 5s), `generatedBy`/`reason`, inspector, polling fallback. Falta evidencia E2E con proveedor real habilitado. |
...
| B.7a | Prueba E2E de proveedor: con `OPENAI_API_KEY` y `LLM_UPGRADE_ENABLED=true`, ejecutar un momento y comprobar un segundo `UI_UPDATED` con `generatedBy: "llm"`; registrar modelo, latencia y fallback en `DECISION_LOG.md` | C + D |
