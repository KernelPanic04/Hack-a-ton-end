...
class ActionPolicy:
    risk: str
    requires_human: bool
    payload_must_be_object: bool = True


ACTION_POLICIES: dict[str, ActionPolicy] = {
    "act_find_alternative": ActionPolicy(risk="medium", requires_human=True),
    "act_accept_delay": ActionPolicy(risk="medium", requires_human=True),
}
...
class ActionPolicyEngine:
    ...
    async def validate(self, event: ActionEvent, run_id: uuid.UUID) -> RunProjection:
        ...
        if policy.payload_must_be_object and not isinstance(event.payload, dict):
            raise PolicyViolation("PAYLOAD_INVALID", "El payload de la acción debe ser un objeto JSON.")
